...
class KoBARTConditionalGeneration(Base):
    def __init__(self, hparams, trainer=None, **kwargs):
        ...
        # self.tokenizer = BartTokenizer.from_pretrained('gogamza/kobart-summarization')
    ...
    def validation_step(self, batch, batch_idx):
        outs = self(batch)
        loss = outs.loss
        # summary_ids = self.model.generate(batch['input_ids'],  num_beams=5,  max_length=512,  eos_token_id=1)
        # output = self.tokenizer.decode(summary_ids.tolist(), skip_special_tokens=True)
        # target = batch['labels']
